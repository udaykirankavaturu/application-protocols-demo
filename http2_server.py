...
# Initialize the Quart application (an async version of Flask)
app = Quart(__name__)

@app.route('/')
async def index():
    """
    Renders the main page. The client-side code is identical to the
    previous demo, but the server's behavior will be different.
    """
    # This HTML and JavaScript is the same as the previous demo.
    # It sends requests to /slow and /fast to test for blocking.
    return """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>HTTP/2 No-Blocking Demo</title>
        <style>
    ...
    <body>
        <div class="container">
            <h1>HTTP/2 No-Blocking Demo</h1>
            <p>This demo uses an HTTP/2 server. Notice how the fast request is <strong>not blocked</strong> by the slow one, thanks to multiplexing.</p>
...
async def fast_request():
    """
    A quick endpoint that will respond immediately because the server
    can handle it concurrently with the /slow request.
    """
    print("Received /fast request. Processing...")
    print("Finished processing /fast request.")
    return "<h1>Fast Request Complete</h1>"

# We don't use the app.run() block here because we are using
# a production-ready ASGI server (Hypercorn) from the command line
# to get HTTP/2 support.